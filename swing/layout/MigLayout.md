...
`MigLayout` 的约束可以通过字符串或链式 API 指定。API 和 String 约束效果是相同的，有些细微差异会在 JavaDoc 中说明。下面主要解释 String 约束格式，API 可以参考 JavaDoc。
...
组件之间的空白区域高度可定制。可以在 row/column 基础上指定每个 cell 之间的 gap。

默认 gap 由平台决定，对应 `"related"` gap 大小。可以在 layout 约束中覆盖整个 layout 的默认 gap，也可以在 row/column 约束中分别指定。
...
gap 后可以添加 `:push`，表示 gap 占据余下所有空间：
...
还有一些特殊的关键词可以使用，如 `"related"`, `"unrelated"`, `"indent"` 和 `"paragraph"`。它们用于更便捷的设置 gaps。

通过创建单位到像素的映射，可以自定义单位。

### Growing and Shrinking

当没有设置组件的 prefSize，其缩放行为高度可定制。

可以将组件划分为增长优先组和缩小优先组，在调整大小时，优先级高的组件先缩放。

可以设置组件的缩放权重，以确定该优先级组中组件去和划分空间。
