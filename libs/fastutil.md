...
AVLTreeMap 和 RBTreeMap 都是标准的二叉平衡树实现，

| 特性         | AVLTreeMap                                                   | RBTreeMap (红黑树)                                           |
| :----------- | :----------------------------------------------------------- | :----------------------------------------------------------- |
| **底层结构** | **AVL树**。一种严格平衡的二叉搜索树，任何节点的左右子树高度差不超过1。 | **红黑树**。一种近似平衡的二叉搜索树，从根到叶子的最长路径不会超过最短路径的2倍。 |
| **性能特点** | **查询更快**。因为树更严格平衡，查找效率更高。适合**查询操作远多于插入/删除**的场景。 | **插入/删除更快**。为了维持平衡，红黑树在插入和删除时需要的结构调整（旋转和颜色变更）比AVL树少。适合**写入操作频繁**的场景。 |
| **内存占用** | 相对较小。每个节点只需要存储平衡因子（如布尔值或字节）。     | 相对较大。每个节点需要额外的空间来存储颜色信息（红/黑）。    |
