...
使用 column 和 row 标题，可插入或删除 column/row，以及设置 column/row 属性。

相对其它 layout-managers，`MigLayout` 有如下优势：

- column/row 中组件默认对齐
- 可以自定义 column width 或 row height 的最小值和最大值
- 支持不同单位：LogicalPixel, Pixel, Point, Millimeter, Centimeter, Inch, Percent and ScreenPercent，其中 LogicalPixel 对创建适配不同屏幕分辨率的自适应界面非常有用
- 可自定义列间距、行间距以及组件间距（gaps）
- 灵活的自动拉伸与收缩规则
- 支持行和列的分组管理
- in-cell flow 支持将多个 component 放入单个 grid-cell
- 将组件 docking 到容器边缘
- 适配 Button Bars 和 Button Order
- 可强制覆盖组件的最小、首选、最大尺寸限制
- 视觉边界优化，修复各类布局异常
- 支持基线对齐

### Insets

所有 MigLayout 容器默认在网格四周带有内边距（insets），效果等同于为容器设置空白边框 `EmptyBorder`。可以在 layout-manager 属性中修改 insets：

- **默认 insets (panel)**

![MigLayout Insets](./images/miglayout_insets.png)

- **0 insets**

![MigLayout no Insets](./images/miglayout_no_insets.png)
...
`MigLayout` 支持在一个 cell 里放置多个组件。这对使用 radioButtonGroup 非常有用，同时避免使用嵌套容器。

- **1 个组件**

![MigLayout in-cell flow 1](./images/miglayout_in_cell_flow1.png)

- **2 个组件**

![MigLayout in-cell flow 2](./images/miglayout_in_cell_flow2.png)

- **3 个组件**

![MigLayout in-cell flow 3](./images/miglayout_in_cell_flow3.png)
...
`MigLayout` 支持将组件停靠在它的边缘（类似 `BorderLayout`）。可以将多个组件停靠在一个边。中间区域则以网格形式排列。

- 顺序：north,west,south,east

![MigLayout Docking](./images/miglayout_docking1.png)

- 顺序：east,south,west,north

![MigLayout Docking](./images/miglayout_docking2.png)

docking 组件的排列顺序由组件田间顺序决定。如上图所示，先添加的组件会获得更大布局空间。在视图中通过拖拽操作可以调整组件的先后顺序。

MigLayout 比 `BorderLayout` 更灵活，且支持 5 个以上组件。组件可以粘附到 5 个位置：`"north"`, `"west"`, `"south"`, `"east"` 以及 `"center"`。中间区域采用正常的网格布局，除非组件设置为 `"dock center"`。docking 用来布局 panels 非常灵活，但也可以用于其它布局需求。

若要 dock 组件，先将其放置在网格任意位置，再右键单击该组件，从右键菜单中选择对应的 dock 选项即可。

![MigLayout Docking Add](./images/miglayout_docking_add1.png)

### Visual Bounds

有些组件的逻辑边界或大于其视觉边界，该问题在 macOS 系统上尤为明显，这会导致在 macOS 中间距过大，而在其它平台中正常。MigLayout 通过在计算组件尺寸时纳入视觉边距进行修正，从而解决该兼容性问题。
