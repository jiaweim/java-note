...
cell 的选择可以通过 `selectionModel` 的 `cellSelectionEnabled` 属性启用。

当 `cellSelectionEnabled` 为 `true`，`TableView` 进入 cell 选择模式，此时不能选择整个 row；如果启用多选模式，依然可以选择整个 row，但是在 cell 选择模式下，`TableView` 不将其作为 row 选择，而是看作多个 cell 选择。cell 选择模式默认为 false，启用方法：
