...
JavaFX 提供 `FileChooser` 和 `DirectoryChooser` 类用于显示文件和目录对话框。

采用对应操作系统原生组件的外观样式，与 JavaFX 自身界面风格相互独立。

在部分文件访问权限受限、或文件操作不属于标准用户交互模型的平台（例如部分移动设备、嵌入式设备）中，调用文件对话框可能不会产生任何实际效果（即返回空文件对象）
...
![](images/Pasted%20image%2020230726113803.png)
